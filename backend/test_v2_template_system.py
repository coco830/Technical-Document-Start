...
def test_v2_template_system():
    ...
    try:
        ...
        print(f"✓ 成功加载文档类型：{list(document_types.keys())}")
        
        for doc_type, doc_info in document_types.items():
            template_id = doc_info.get("template_id")
            template_path = generator.get_template_path(template_id)
            ai_sections = generator.get_template_ai_sections(template_id)
            print(f"  - {doc_type}: {doc_info.get('output_name')}")
            print(f"    模板ID: {template_id}")
            print(f"    模板路径: {template_path}")
            print(f"    AI段落: {ai_sections}")
    except Exception as e:
        print(f"错误：模板注册表加载失败 - {str(e)}")
        return False
    
    # 测试生成所有文档
    print("\n开始生成所有文档...")
    try:
        result = generator.generate_all_documents(enterprise_data, "test_user", use_v2=True)
        ...
        if result["success"]:
            ...
            for doc_type, content in result.items():
                ...
                if content:
                    file_name = f"{doc_type}_v2_demo.html"
                    file_path = output_dir / file_name
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    print(f"✓ {doc_type}已保存：{file_path}")
            
            # 保存AI段落信息
            sections_file = output_dir / "ai_sections_v2_demo.json"
            with open(sections_file, 'w', encoding='utf-8') as f:
                json.dump({section: result["ai_sections"].get(section, "") 
                         for section in result["ai_sections_used"]}, 
...
def test_single_document_generation():
    """测试单个文档生成"""
    print("\n" + "=" * 60)
    print("单个文档生成测试")
    print("=" * 60)
    ...
    for doc_type, doc_info in document_types.items():
        print(f"\n测试生成{doc_info.get('output_name', doc_type)}...")
        try:
            result = generator.generate_single_document(doc_type, enterprise_data, "test_user", use_v2=True)
            ...
            if result["success"]:
                ...
                file_name = f"{doc_type}_single_v2_demo.html"
                file_path = output_dir / file_name
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(result["content"])
                print(f"✓ {doc_type}已保存：{file_path}")
            else:
                print(f"错误：{doc_type}生成失败 - {result['errors']}")
        except Exception as e:
            print(f"错误：{doc_type}生成过程中发生异常 - {str(e)}")
    
    return True

if __name__ == "__main__":
    success1 = test_v2_template_system()
    success2 = test_single_document_generation()
